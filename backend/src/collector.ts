...
async function fetchWithRetry(url: string, params: Record<string, string | number>, retries: number = G2B_API.RETRY_COUNT): Promise<any> {
...
export async function collectBids(options?: {
  ...
  // 중복 제거 후 제외 키워드 필터링 및 DB 저장
  for (const [ntceNo, bid] of bidMap) {
    if (isExcluded(bid.bid_ntce_nm || '')) {
      result.excludedCount++;
      continue;
    }

    const isNew = upsertBid(bid);
    if (isNew) {
      result.newBids++;
    }
  }
  ...
  if (result.errors.length > 0) {
    console.log(`⚠️ 에러 ${result.errors.length}건`);
  }
...
if (require.main === module) {
  import('./db').then(({ initDB }) => {
    initDB();
    collectBids().then(r => {
      console.log('수집 완료:', JSON.stringify(r, null, 2));
      process.exit(0);
    });
  });
}
