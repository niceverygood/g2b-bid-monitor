...
function buildSlackMessage(bid: Bid) {
  ...
  const keyPoints = (bid.key_points_json || []) as string[];
  const risks = (bid.risks_json || []) as string[];
  ...
  return {
    blocks: [
      ...
      { type: 'section', text: { type: 'mrkdwn', text:
        ...
        `💰 추정가격: ${formatPrice(bid.presmpt_prce)}\n` +
        `📅 마감: ${bid.bid_clse_dt || '-'}\n` +
...
async function sendSlack(payload: any, retries: number = 2): Promise<boolean> {
  if (!ENV.SLACK_WEBHOOK_URL || ENV.SLACK_WEBHOOK_URL.includes('여기에')) return false;
...
export async function notifyNewBids(): Promise<number> {
  const bids = await getUnnotifiedBids(SCORE_THRESHOLD.SLACK_NOTIFY);
  let sent = 0;

  for (const bid of bids) {
    const ok = await sendSlack(buildSlackMessage(bid));
    if (ok) {
      await markNotified(bid.bid_ntce_no);
      sent++;
    }
    await sleep(500);
  }

  if (sent > 0) console.log(`📢 Slack 알림 ${sent}/${bids.length}건 발송`);
  return sent;
}
...
export async function notifyPipelineResult(result: PipelineResult): Promise<boolean> {
  ...
  const recPrice = result.priceAdvice ? formatPrice(result.priceAdvice.recommended_bid_price) : '-';
  ...
  const payload = {
    blocks: [
      ...
      { type: 'section', text: { type: 'mrkdwn', text:
        ...
        `💰 추정가: ${formatPrice(bid.presmpt_prce)} | 📅 마감: ${bid.bid_clse_dt || '-'}`,
      }},
      { type: 'section', text: { type: 'mrkdwn', text:
...
export async function sendDeadlineReminder(): Promise<number> {
  const urgentBids = await getDeadlineAlertBids(2);
