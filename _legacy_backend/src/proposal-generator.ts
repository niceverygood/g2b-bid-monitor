...
export async function generateAllProposals(bidId: number, onProgress?: (result: GenerationResult) => void): Promise<GenerationResult[]> {
  const results: GenerationResult[] = [];
  const { saveProposal } = await import('./db');
  const bid = getBidById(bidId);
  if (!bid) throw new Error('공고를 찾을 수 없습니다');
  ...
  for (const [docType, label] of Object.entries(DOC_TYPES)) {
    try {
      ...
      const result: GenerationResult = { docType: docType as DocType, label, success: true };
      results.push(result);
      onProgress?.(result);
      console.log(`  ✅ ${label} 완료`);
      await sleep(1000);
    } catch (error: any) {
      const result: GenerationResult = { docType: docType as DocType, label, success: false, error: error.message };
      results.push(result);
      onProgress?.(result);
